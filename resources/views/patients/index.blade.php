...
<a href="{{route('patients.create')}}" class="create-button">Crear</a>

<table class="table-list patients-table">
  <thead>
    <tr>
      <td>Nombre</td>
      <td>Edad</td>
      <td>Documento indentidad</td>
      <td>Direccion</td>
      <td>Opciones</td>
    </tr>
  </thead>
  @foreach ($patients as $patient)
    @if ($patient->status==='active')
      <tr class="table-card">
        <td>{{$patient->name}}</td>
        <td>{{$patient->age}}</td>
        <td>{{$patient->identity_document}}</td>
